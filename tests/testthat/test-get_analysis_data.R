...
testthat::test_that("Correct VMT download is returned", {
  ...
  testthat::expect_equal(names(an_dat), c("data_periods", "mode_of_travel", 
...
testthat::test_that("Correct VMT sample size is returned", {
  ...
  testthat::expect_equal(names(an_dat), 
                         c("data_periods", "mode_of_travel", 
...
testthat::test_that("Correct za_all is returned", {
  ...
  testthat::expect_equal(ncol(an_dat), 15)
  testthat::expect_equal(names(an_dat), 
                         c("data_periods", "mode_of_travel", "intersection_type", "zone_id", 
  ...
  get_analysis_data(
    ...
    testthat::expect_error("`metric` 'cats' is unavailable
`metric` must match one of the available metrics for this analysis
za_all, prediction_interval, sample_size")
